...
@section('content')
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-light text-gray-800 border-b pb-4">All Reviews & Ratings</h2>
            <p class="text-gray-600 mt-2">Manage and monitor all user reviews</p>
        </div>
...
\    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 mb-8">
        <form action="{{ route('admin.reviews.all') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Tourism Object</label>
                <select name="tourism_object_id" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-teal-500">
                    <option value="">All Tourism Objects</option>
    ...
    <div class="space-y-4">
        @forelse($reviews as $review)
            <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition">
                ...
                @if($review->tourismObject)
                    <div class="mb-3 inline-flex items-center px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-medium">
                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                        </svg>
                        {{ $review->tourismObject->name }}
                    </div>
                @endif
